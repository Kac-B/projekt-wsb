...
def films_reco(request, id, model):
    ...
    popular_movie_list = [movie_dict[movie.movieid_id] for movie in popular_movies[:6]]
    data['recommendation'] = get_recommendation(request )# rekomendacja według gatunku
...
def get_recommendation(request):#, popular_movie_list
    ...
    for item in sorted_list:
        ...
        if len(result) == 6:
            break
    return [result[i] for i in random.sample(range(len(result)), 6)]


@csrf_protect
def war_reco(request, id, model):
    data = {}
    movie_dict = search_index.data_in_memory['movie_dict']
    popular_movies = Popularity.objects.all().order_by('-weight')
    popular = []
